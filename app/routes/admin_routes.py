...
from app.services.books import get_books_by_section, get_all_user_issued_books, add_book, get_sections, add_section, \
    ...
    get_all_current_user_books, get_all_requested_books, approve_request
...
def get_user_assigned_books(admin_id, section_id):
    ...
    if is_add_book:
        return render_template('admin_section_books.html', admin_id=admin_id, section_id=section_id, books=books_list,
                               add_new_book=True, section_name=section_name)
    if edit_book_id:
        return render_template('admin_section_books.html', admin_id=admin_id, section_id=section_id, books=books_list,
                               add_new_book=False, section_name=section_name, edit_book_id=int(edit_book_id))
    return render_template('admin_section_books.html', admin_id=admin_id, section_id=section_id, books=books_list,
                           add_new_book=False, section_name=section_name)
...
def add_new_book(admin_id, section_id):
    ...
    if added:
        ...
        return render_template('admin_section_books.html', admin_id=admin_id, section_id=section_id,
...
def list_sections(admin_id, justAdded=False):
    ...
    if edit_section_id:
        return render_template('admin_sections.html', admin_id=admin_id, sections=section_list, add_section=False,
                               edit_section_id=int(edit_section_id))
    return render_template('admin_sections.html', admin_id=admin_id, sections=section_list, add_section=False)
...
@admin_bp.route('/admins/<string:admin_id>/sections/<string:section_id>/books/<string:book_id>/deleteBook',
                methods=['GET'])
def delete_book_get(admin_id, section_id, book_id):
    ...
    if success:
        ...
        return render_template('admin_section_books.html', admin_id=admin_id, section_id=section_id,
                               error='Book Delete Issue')


@admin_bp.route('/admins/<string:admin_id>/sections/<string:section_id>/books/<string:book_id>/updateBook',
                methods=['POST'])
def update_book_post(admin_id, section_id, book_id):
    ...
    if success:
        ...
        return render_template('admin_section_books.html', admin_id=admin_id, section_id=section_id,
                               error='Book Update Issue')


@admin_bp.route('/admins/<string:admin_id>/revokeBook/<string:user_requested_books_id>', methods=['GET'])
@admin_bp.route(
    '/admins/<string:admin_id>/sections/<string:section_id>/books/<string:book_id>/revokeBook/<string:user_requested_books_id>',
    methods=['GET'])
...
@admin_bp.route(
    '/admins/<string:admin_id>/requests/revokeBook/<string:user_requested_books_id>',
    methods=['GET'])
def revoke_book_status_get(admin_id, user_requested_books_id):
    success = revoke_book(user_requested_books_id)

    if not success:
        return render_template('admin_dashboard.html', admin_id=admin_id, error='Book Revoke Issue')
    return redirect(f"/admins/{admin_id}/requests")
...
def get_requests_view(admin_id):
    ...
    request_id= request.args.get('view_request')
    view_book_status_id = request.args.get('view_book_status')
    if not request_id and not view_book_status_id:
        return render_template('admin_requests.html', admin_id=admin_id, book_statuses_list=book_list, requests=request_list)
    if view_book_status_id:
        return render_template('admin_requests.html', admin_id=admin_id, book_statuses_list=book_list, requests=request_list, view_book_status_id=int(view_book_status_id))
    return render_template('admin_requests.html', admin_id=admin_id, book_statuses_list=book_list, requests=request_list, view_request_id=int(request_id))
@admin_bp.route('/admins/<string:admin_id>/requests/approve_request/<string:request_id>', methods=['GET'])
def approve_request_get(admin_id, request_id):
    is_approved= request.args.get('approve')
    success = approve_request(request_id, is_approved)
    if not success:
        return render_template('admin_requests.html', admin_id=admin_id, error='Request Approval Issue')
    return redirect(f"/admins/{admin_id}/requests")