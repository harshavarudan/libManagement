...
def get_books_read_per_section(user_id):
    ...
    cur = con.cursor()
    cur.row_factory = sqlite3.Row
    cur.execute("SELECT sections.title AS section_title, COUNT(user_books.book_id) AS books_read FROM user_books JOIN books ON user_books.book_id = books.id JOIN sections ON books.book_section = sections.id WHERE user_books.user_id = ? GROUP BY sections.title",(user_id,))
    data = cur.fetchall()
    con.close()
    return data
...
def return_book(user_id, book_id):
    con = sqlite3.connect(DB_PATH)
    cur = con.cursor()
    cur.execute("UPDATE user_books SET returned_on = CURRENT_TIMESTAMP WHERE user_id = ? AND book_id = ?", (user_id, book_id))
    con.commit()
    con.close()
...
def update_section(section_id,title,description,date):
    ...
    con = sqlite3.connect(DB_PATH)
    cur = con.cursor()
    cur.execute("UPDATE sections SET title=?,description=?,date_created=? WHERE id=?", (title,description,date,section_id))
    con.commit()
    con.close()
    return True
...
def get_all_current_user_books():
    ...
    cur = con.cursor()
    cur.row_factory = sqlite3.Row
    cur.execute("select books.title as book_title, users.name as user_name, user_books.id   from user_books   join books on user_books.book_id = books.id   join users on user_books.user_id = users.id         where user_books.returned_on is NULL")
    data = cur.fetchall()
    con.close()
    return data
def get_all_requested_books():
    ...
    cur = con.cursor()
    cur.row_factory = sqlite3.Row
    cur.execute("SELECT * FROM user_requested_books where approved_on is NULL")
    data = cur.fetchall()
    con.close()
    return data
